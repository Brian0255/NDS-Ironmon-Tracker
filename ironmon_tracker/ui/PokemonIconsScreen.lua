local function PokemonIconsScreen(initialSettings, initialTracker, initialProgram)
    ...
    local function setAuthorLabel()
        local author = currentIconSet.AUTHOR
        local labelName = "Credit: "..author
        ui.controls.authorLabel.setText(labelName)
        local offset = {
            x = getCenterStart(ui.controls.authorLabel.getSize().width-2, labelName),
            y = 1
        }
        ui.controls.authorLabel.setTextOffset(offset)
    end

    local function randomizeIcon()
        changeIndexToRandomID()
        DrawingUtils.readPokemonIDIntoImageLabel(currentIconSet, currentPokemonIndex, ui.controls.pokemonImageLabel, currentIconSet.CHOOSE_IMAGE_OFFSET)
