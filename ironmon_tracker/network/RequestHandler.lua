RequestHandler = {
	...
	SAVED_REQUESTS = "Requests.json", -- Filename where requests are saved between game sessions
...
function RequestHandler.IRequest:new(o)
	o = o or {}
	o.GUID = o.GUID or NetworkUtils.newGUID()
...
function RequestHandler.addUpdateRequest(request)
	...
	if RequestHandler.Requests[request.GUID] then
		RequestHandler.Requests[request.GUID] = MiscUtils.deepCopy(request)
	else
...
function RequestHandler.addUpdateResponse(response)
	...
	if RequestHandler.Responses[response.GUID] then
		RequestHandler.Requests[response.GUID] = MiscUtils.deepCopy(response)
	else
